                    ...
                    foreach ($custom_field_mappings as $ActiveCampaignFieldKey => $customFieldKey) {
                        ...
                        if ( ! empty($customFieldKey) && ! empty($this->extras[$customFieldKey])) {
                            switch ($ActiveCampaignFieldKey) {
                                case 'phone':
                                    $contact['phone'] = esc_attr($this->extras[$customFieldKey]);
                                    break;
                                case 'orgname':
                                    $contact['orgname'] = esc_attr($this->extras[$customFieldKey]);
                                    break;
                                default :
                                    $contact[sprintf('field[%s,0]', $ActiveCampaignFieldKey)] = esc_attr($this->extras[$customFieldKey]);
                                    break;
                            }
                        }
                    }
